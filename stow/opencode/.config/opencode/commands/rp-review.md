...
Focus: $ARGUMENTS
If no focus was provided, check for bugs, regressions, unsafe behavior, missing validation, and test gaps.
...
4. Return findings by severity with file paths and concrete fixes in chat or the requested Plannotator browser/TUI surface.
5. If there are no issues, say so plainly.
6. Apply review-driven revisions only when the user requests them.
